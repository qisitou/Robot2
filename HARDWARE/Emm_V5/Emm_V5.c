...
/**
  * @brief    读取电机位置
  * @param    addr  ：电机地址
  * @param	  mode  ：1返回角度，0返回编码器原始读数
  * @retval   
  */

float pos = 0.0f, Motor_Cur_Pos = 0.0f;
float Read_Motor_Pos(uint8_t addr, uint8_t mode)
{
	can.rxFrameFlag = false;

	Emm_V5_Read_Sys_Params(addr, S_CPOS);
	while(can.rxFrameFlag == false); can.rxFrameFlag = false;
	if(can.CAN_RxMsg.Data[0] == 0x36 && can.CAN_RxMsg.DLC == 7)
	{
	// 拼接成uint32_t类型
		pos = (uint32_t)(
						((uint32_t)can.CAN_RxMsg.Data[2] << 24)		|
						((uint32_t)can.CAN_RxMsg.Data[3] << 16)  	|
						((uint32_t)can.CAN_RxMsg.Data[4] << 8)   	|
						((uint32_t)can.CAN_RxMsg.Data[5] << 0)
						);
		// 转换成角度
		Motor_Cur_Pos = (float)pos * 360.0f / 65536.0f;
		//printf("pos = %f\n",pos);
		//符号
		if(can.CAN_RxMsg.Data[1]) { Motor_Cur_Pos = -Motor_Cur_Pos; pos = -pos;}
		//printf("Motor_Cur_Pos = %f\n",Motor_Cur_Pos);
	}
	if(mode)
	{
